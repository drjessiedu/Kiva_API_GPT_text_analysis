...
STARTING_DATE_ONE = datetime.datetime(2024, 8, 1)
ENDING_DATE_ONE = datetime.datetime(2024, 9, 30)
    ...
    while slice_beginning < len(id_list):
        ...
        for value in data:
            ...
            while lendingActionOffset < totalCountLendingActions:
                try:
                    graphql_query_2 = f'{{lend {{loans(filters: {{loanIds: {[loan_id]}, status: all }}) {{values {{lendingActions(offset:{lendingActionOffset}, limit: 100){{totalCount values {{ shareAmount latestSharePurchaseDate lender {{id publicId name loanCount inviteeCount' \
